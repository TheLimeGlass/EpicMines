...
public class Utils {
	...
	private static JarFile getJar(EpicMines instance) {
		try {
	...
	public static void loadClasses(EpicMines instance, String basePackage, String... subPackages) {
	...
	public static <T> List<Class<T>> getClassesOf(EpicMines instance, String basePackage, Class<T> type) {
		JarFile jar = getJar(instance);
		if (jar == null)
			return null;
		basePackage = basePackage.replace('.', '/') + "/";
			...
			for (Enumeration<JarEntry> jarEntry = jar.entries(); jarEntry.hasMoreElements();) {
				...
				if (name.startsWith(basePackage) && name.endsWith(".class")) {
					...
					try {
						...
						EpicMines.consoleMessage("Class " + className + " was formatted incorrectly, report this to the developers or an addon developer for Kingdoms.");
						e.printStackTrace();
					}
					if (clazz == null)
						continue;
